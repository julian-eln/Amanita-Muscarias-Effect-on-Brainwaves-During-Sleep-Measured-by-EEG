...
The next major measurement was relative spectral power (how much of the total power each band makes up). Delta dominates in both conditions. Delta climbed into the 70–85% range in stretches of both nights, especially late in the treatment session after the tincture. The band that clearly changed is alpha, which was reduced in the treatment (about 3.9% vs 5.1% in the baseline). Theta was slightly higher in the treatment rather than suppressed, and gamma was lower in the treatment, not higher (the baseline carried more high-frequency power). Beta was similar in both.
...
So, despite my original assumptions, there was a mild, broad, non-significant lowering of power across all bands in the treatment. In relative terms, delta dominated both nights and trended higher in the treatment (about 69% vs 63% of total power), consistent with animal studies on Amanita muscaria (Scotti de Carolis et al., 1969), but this difference was not significant (Kruskal-Wallis p = .109). The workbook test flags alpha as lower in the treatment (about 5.1% to 3.9%, p = .023), and Beta and Gamma ranked higher in the control (p = .039 / .036), but all three effects are specific to the dB-averaging aggregation — when recomputed from raw with standard per-epoch-relative aggregation, none is significant (Alpha p = .949, Beta p = .927, Gamma p = .670). Relative sigma did not differ (p = .430) and theta showed a non-significant upward trend. So the honest bottom line is no robust spectral difference between the two nights in any band. When looking at the EEG through topographic maps, sigma was spread out but noticeably less in the parietal lobe during the treatment and a bit more present in the frontal lobe, which is associated with reasoning and planning. These findings may represent a potential mechanism underlying the dissociative and hallucinatory effects of Amanita muscaria, although substantially more data are required before firm conclusions can be drawn. 

The fact that not only did beta and gamma trend down and there was a trend down across all bands  throughout the treatment show a potenitally big difference between Amanita and other GABA drugs in brainwave effects.


On the relative data, normality testing made Theta and Alpha parametric and Delta, Beta, Gamma, and Sigma non-parametric. Independent-samples t-tests found Alpha significantly lower in the treatment (p = .023) and Theta non-significant (p = .338). Kruskal-Wallis tests found Delta (p = .109) and Sigma (p = .430) non-significant, and Beta (p = .039) and Gamma (p = .036) higher in the control by rank — but all three significant effects disappear under standard per-epoch-relative aggregation. For the absolute (dB) values, no individual band differed significantly between conditions. Because each of these tests treats 5-minute bins as independent samples (pseudoreplication) from a single participant with one session per condition, they should be read as a single-subject, exploratory result, not a population-level finding.  

## Limitations:

 The Muse is a consumer headset, meaning it is not as advanced as clinician-grade EEGs and doesn't have as many sensors as even more modern (post-2014) consumer EEG systems. It only has 4 sensors, substantially fewer than the standard 10-20 EEG system described by Jasper (1957).   There are also major artifacts, like at 85 minutes, some of the spikes may be due to movement, and because I set alarms in the treatment to know when I should mark down the time certain effects started, which may have introduced additional artifacts.  
